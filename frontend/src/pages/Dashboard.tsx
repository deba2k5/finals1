...
import { satelliteService, SatelliteData } from '@/services/satelliteService'; // Keep these imports
import { weatherService, ForecastDay } from '@/services/weatherService';     // Keep these imports
...
const Dashboard = () => {
    ...
    if (location) {
      (async () => {
        try {
          ...
          setWeatherData(prev => ({
            temperature: `${weather.temperature}°C`, // FIXED: Use backticks
            humidity: `${weather.humidity}%`,       // FIXED: Use backticks
            // FIXED: Use optional chaining for forecast[0] and wrap in backticks
  ...
  const fetchData = async (lat: number, lon: number) => {
    ...
    try {
      ...
      setFarmData({
        // FIXED: Use backticks and correctly access properties
        ...
        // FIXED: Use backticks for yieldPrediction
  ...
  useEffect(() => {
    ...
    forecast.forEach(day => {
      if (day.rain >= 80) {
        alerts.push({
          id: `rain-${day.day}`, // FIXED: Use backticks
          type: 'weather',
          severity: 'high',
          message: `${day.day}: ${t('heavy_rain_expected', { rain: day.rain })}`, // FIXED: Use backticks
          time: 'Upcoming'
        });
      } else if (day.rain >= 50) {
        alerts.push({
          ...
          type: 'weather',
          severity: 'medium',
          message: `${day.day}: ${t('moderate_rain_expected', { rain: day.rain })}`, // FIXED: Use backticks
          time: 'Upcoming'
        });
      }
      if (day.condition.toLowerCase().includes('storm')) {
        alerts.push({
          id: `storm-${day.day}`, // FIXED: Use backticks
          type: 'weather',
          severity: 'high',
          message: `${t('storm_expected')}`, // FIXED: Use backticks
          time: 'Upcoming'
        });
      }
    });
    ...
    if (satelliteData) {
      ...
      if (satelliteData.geospatialAnalysis?.healthAnalysis?.stressAreas?.length > 0) {
        satelliteData.geospatialAnalysis.healthAnalysis.stressAreas.forEach((area, idx) => {
          alerts.push({
            id: `stress-${idx}`, // FIXED: Use backticks
            type: 'disease',
            severity: area.severity,
            // FIXED: Use backticks for message
  ...
  }, [forecast, satelliteData, t]); // Added 't' to dependency array for i18n functions
    ...
    <div className="container mx-auto p-4 md:p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="agri-card">
          ...
          <CardContent>
            {/* Dynamically render alerts from recentAlerts state */}
            {recentAlerts.length > 0 ? (
              recentAlerts.map(alert => (
                <div key={alert.id} className="flex items-start space-x-3 p-3 rounded-lg bg-secondary/50 mb-2 last:mb-0">
              ...
              <p className="text-center text-muted-foreground">{t('no_new_alerts')}</p>
            )}
            {/* Removed the hardcoded alert examples, as they should be rendered from recentAlerts state */}
          </CardContent>
        </Card>
